...
"""
Lavender image steganography tool.
...
def main():
    ...
    _, command, file, image = argv
    
    codec = ImageSteganography()
    
    if command == 'encode':
        image_file = Image.open(image)
        message = open(file, 'rb').read()
        
        codec.encode(message, image_file).save(f'./{image}-encoded.png')
    
    elif command == 'decode':
        image_file = Image.open(image)
        message = open(file, 'wb')
        message.write(codec.decode(
